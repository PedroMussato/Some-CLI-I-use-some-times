...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--encrypt",
        action="store_true",
        help="Encrypt file/directory into tar.xz.enc",
    )
    parser.add_argument(
        "-d", "--decrypt", action="store_true", help="Decrypt tar.xz.enc and extract"
    )
    parser.add_argument(
        "-o", "--output", help="Custom output filename (for tar or decrypted tar)"
    )
    parser.add_argument(
        "-t", "--target", default=".", help="Target directory for extraction"
    )
